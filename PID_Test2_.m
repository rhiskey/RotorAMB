...
t0 = 0; tend =0.01; %с, время для решателя ode 0.001
...
%-----Размерные параметры------------------
nu=1e4; % rpm 1e4 Частота вращения двигателя В Об.сек = w=omega  1*10000
...
gamma=2.15*1e-5; %2.15*1e-5; %m2*e^2; %Дебаланс, тут m- масса ротора! 2.15*1e-5 рад
...
%Управляющие токи АМП в соответствующих направлениях, меняем жёсткость системы
i0=0.00014;%0.000001;  %A,i_0=i_max/2 ??? 0.014
...
x1=h/2;
y1=h/4;
alpha1=0; %0.02 -max угол наклона ротора
beta1=0; %0.015
vx1=0; %0.1
vy1=0; %0.2
om1=0;
...
%Безразмерные токи
i0_dim=i0/Si; %Ток смещения 
iax_dim=ia.x/Si; %управляющие токи  0,541; 5,145e-5; 54.144; max  5,42 
iay_dim=ia.y/Si;
ibx_dim=ib.x/Si;
iby_dim=ib.y/Si;
...
% P2=(m*l^2)/Ix; %8.714
P2=8.714;
% Pk=ka*P_e/(cm*R_e*delta_dim^3); %конструктивные параметры АМП 10e5
Pk =10e5;
%%----------Постоянные матрицы---------------
...
Zb=[1 0 0 -a1_dim;
    ...
    1 0 0 a2_dim;
    0 1 -a2_dim 0];
%обратная матриа перехода
Zb_1=pinv(Zb);
%---------------------------------
%Для вектора состояния  Z'=AZ+B
...
Test2_
Test2
% k.sab, k.iab - коэффициенты управления, m2-масса ротора
%коэфф усиления:
% P.a=(-2*k.sab/k.iab);
% P.a=P.b;
